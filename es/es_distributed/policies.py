...
class Policy:
    ...
    def rollout(self, env, render=False, timestep_limit=None, save_obs=False, random_stream=None):
        ...
        """
        env_timestep_limit = self.get_env_timestep_limit(env)
        ...
        for _ in range(timestep_limit):
            ob = np.array(ob) # assure, that observation is a numpy array
    ...
    def get_env_timestep_limit(self, env):
...
def bins(x, dim, num_bins, name):
    ...
    return tf.argmax(scores_nab, 2)  # 0 ... num_bins-1 - wyjsciem jest wektor nr kubelkow, ktore uzyskaly max dla poszczegolnych wymiarow


class HumanoidPolicy(Policy):
    def _initialize(self, ob_space, ac_space, ac_bins, ac_noise_std, nonlin_type, hidden_dims, connection_type):
        ...
        :param ac_space: action space; for walker2d: 6-d vector with elements from [-1,1]
        :param ac_bins: czy akcje dyskretne/ciagl; gdy dyskretne, mozna podac uniform:liczba_kubelkow
        :param ac_noise_std: std szumu dodawanego do akcji
        :param nonlin_type: typ nieliniowosci na warstwach ukrtych
        :param hidden_dims: liczba jednostek na warstwach ukrytych
        :param connection_type: ff for fully connected
        :return:
        """
        ...
        with tf.variable_scope(type(self).__name__) as scope:
            ...
            a = self._make_net(tf.clip_by_value((o - ob_mean) / ob_std, -5.0, 5.0)) ## TODO(osim-rl): clipping and normalization of observation values may not work properly with osim-rl
    ...
    def _make_net(self, o):
        ...
        assert isinstance(self.ac_bins, basestring)
        ac_bin_mode, ac_bin_arg = self.ac_bins.split(':')

        if ac_bin_mode == 'uniform':
            # Uniformly spaced bins, from ac_space.low to ac_space.high
            # czyli np. dla zakresu akcji [-1,1] i 5 kubelkow, wartosci sa nastepujace(krok co 0.4): -1, -.6, -.2,...,1
            ...
            a = U.dense(x, adim, 'out', U.normc_initializer(0.01)) # TODO(osim-rl) wyjscia sa z zakresu [-inf, inf]
        else:
...
class OsimRunnerPolicy(HumanoidPolicy):
    def get_env_timestep_limit(self, env):
        return env.spec.timestep_limit
