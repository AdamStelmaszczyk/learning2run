...
def dense(x, size, name, weight_init=None, bias=True):
    ...
    ret = tf.matmul(x, w)
    if bias:
        b = tf.get_variable(name + "/b", [size], dtype=tf.float32, initializer=tf.constant_initializer(0.0))
        return ret + b
    else:
        return ret
...
def function(inputs, outputs, updates=None, givens=None):
    """
    Creates function with _call__(inputvals) (see below)
    This function takes input values, puts them into placeholders,
    calulates output values and returns them.

    :param inputs: input placeholders
    :param outputs: output function
    :param updates: procedures updating (return void)
    :param givens: data to feed for input
    :return:
    """
