...
Result = namedtuple('Result', [
    'worker_id',
    'noise_inds_n', 'returns_n2', 'signreturns_n2', 'lengths_n2', 'rollout_times',
    ...
    while True:
        ...
        while num_episodes_popped < config.episodes_per_batch or num_timesteps_popped < config.timesteps_per_batch:
            ...
            if result.eval_length is not None:
                ...
                rollout_times = np.append(rollout_times, result.rollout_times)
        ...
        tlogger.record_tabular("RolloutTimeMean", np.mean(rollout_times))
        tlogger.record_tabular("RolloutTimeSum", np.sum(rollout_times))
...
def rollout_and_update_ob_stat(policy, env, timestep_limit, rs, task_ob_stat, calc_obstat_prob):
    """
    Performs one rollout - one full episode
    (start from restarted environment, do action till the env says 'done' - this is one rollout).
    :return (list of rewards for each action), number of steps done in episode
    """
    ...
    while True:
        ...
        if rs.rand() < config.eval_prob:
            ...
            worker.push_result(task_id, Result(
                ...
                signreturns_n2=None,
                lengths_n2=None,
                rollout_times=np.array([rollout_tend-rollout_tstart]),
            ...
            while not noise_inds or time.time() - task_tstart < min_task_runtime:
                ...
                rollout_pos_tstart = time.time()
                rews_pos, len_pos = rollout_and_update_ob_stat(
                    policy, env, task_data.timestep_limit, rs, task_ob_stat, config.calc_obstat_prob)
                rollout_pos_time = time.time() - rollout_pos_tstart

                # jeden worker wykonuje dwa rollout'y, symetrycznie
                policy.set_trainable_flat(task_data.params - v)
                rollout_neg_tstart = time.time()
                rews_neg, len_neg = rollout_and_update_ob_stat(
                    policy, env, task_data.timestep_limit, rs, task_ob_stat, config.calc_obstat_prob)
                rollout_neg_time = time.time() - rollout_neg_tstart

                noise_inds.append(noise_idx)
                returns.append([rews_pos.sum(), rews_neg.sum()])
                signreturns.append([np.sign(rews_pos).sum(), np.sign(rews_neg).sum()])
                lengths.append([len_pos, len_neg])
                rollout_times = np.append(rollout_times, rollout_pos_time)
                rollout_times = np.append(rollout_times, rollout_neg_time)
            ...
            worker.push_result(task_id, Result(
                ...
                rollout_times=np.array(rollout_times, dtype=np.float32),
                eval_return=None,
                eval_length=None,
